...
EmotesListController = RouteController.extend({
  ...
  subscriptions: function() {
    this.emotesSub = Meteor.subscribe('emotes', this.findOptions());
  },
  emotes: function() {
...
Router.route('/emotes/:_id', {
  ...
  waitOn: function() {
    return [
      Meteor.subscribe('singleEmote', this.params._id),
...
Router.route('/emotes/:_id/edit', {
  name: 'emoteEdit',
  waitOn: function() { 
    return Meteor.subscribe('singleEmote', this.params._id);
  },
  data: function() { return Emotes.findOne(this.params._id); }
});
